# /jpspec:security Architecture - Strategic Framing and System Design
...
| **Architectural Fit** | Agent coordination mismatch | Native `/jpspec` workflow | Native ✓ |
...
After:  Code → /jpspec:security scan+triage+fix → Review patches → Apply → Rescan
```
...
- CLI flags consistent with existing `/jpspec` commands (`--format`, `--verbose`)
- State transitions integrate with jpspec_workflow.yml
...
**Strengths:**
- Progressive disclosure: `/jpspec:security scan` → triage → fix → audit
...
- [ ] `/jpspec:security scan` command
- [ ] `/jpspec:security triage` command
...
- [ ] `/jpspec:security fix` command (AI patch generation)
...
**Mandate:** Projects MUST be able to use `/jpspec` workflow without `/jpspec:security`.
...
| `/jpspec:security` adoption | >80% of projects | 6 weeks |
...
- **PRD:** `docs/prd/jpspec-security-commands.md` - Product requirements
- **Assessment:** `docs/assess/jpspec-security-commands-assessment.md` - Feature assessment
...
- task-215: Implement /jpspec:security CLI Commands
...
2. Create detailed ADRs for each architectural decision:
   ...
   - ADR-007: Unified Security Finding Format
   - ADR-008: Security MCP Server Architecture
3. Update task-227 implementation plan
