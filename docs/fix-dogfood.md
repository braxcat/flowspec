...
**What it does NOT do:**
- Does NOT create symlinks for `jpspec` commands
...
```
.claude/commands/
├── jpspec.implement.md      (2933 bytes)
├── jpspec.operate.md        (2796 bytes)
├── jpspec.plan.md           (2266 bytes)
├── jpspec.research.md       (1491 bytes)
├── jpspec.specify.md        (1534 bytes)
├── jpspec.validate.md       (3017 bytes)
...
- **17 command files** (8 speckit symlinks + 9 jpspec real files)
- **Subdirectory structure** (not flat with dot notation)
- speckit files are symlinks; jpspec files are NOT symlinks
- jpspec files are **heavily customized** with backlog integration

### Step 3: The Three Versions Problem

There are **THREE different versions** of jpspec commands:

| Location | Files | Content |
|----------|-------|---------|
| `nanofuse/.claude/commands/jpspec.*.md` | 6 files | From release package, small/simple |
| `jp-spec-kit/templates/commands/jpspec/` | 6 files | Templates for release, small/simple |
| `jp-spec-kit/.claude/commands/jpspec/` | 9 files | Enhanced for development, 3-7x larger |
...
**Impact**: The jpspec commands in `.claude/commands/jpspec/` are manually maintained separately and have diverged significantly from the templates.

### Issue 2: jpspec Content Divergence (CRITICAL)

**Problem**: The jpspec commands used for jp-spec-kit development are 3-7x larger than the template versions, with extensive backlog integration that is NOT distributed to end users.
...
### Issue 3: Missing jpspec Commands in Templates

**Problem**: Three jpspec commands exist in `.claude/commands/jpspec/` but NOT in templates:
- `assess.md`
...
### Fix 1: Sync jpspec Commands

Either:
- **Option A**: Copy enhanced `.claude/commands/jpspec/` files to `templates/commands/jpspec/`
- **Option B**: Make `.claude/commands/jpspec/` use symlinks to templates (like speckit)
...
```python
# Create jpspec symlinks as well
...
for template_file in jpspec_templates_dir.glob("*.md"):
...
- Add `assess.md` to `templates/commands/jpspec/`
- Add `_backlog-instructions.md` to `templates/commands/jpspec/`
- Add `prune-branch.md` to `templates/commands/jpspec/`
