...
def voice(
    ...
    """Launch voice assistant for JP Spec Kit.

    The voice assistant provides real-time voice interaction with JP Spec Kit
    using speech-to-text, language models, and text-to-speech.

    Requires API keys set in environment variables (see templates/voice-config.json).
    """
    from pathlib import Path
    ...
    show_banner()
    console.print("[bold]JP Spec Kit Voice Assistant[/bold]\n")
    ...
    if config:
        ...
        config_path = Path("voice-config.json")
        if not config_path.exists():
            console.print("[red]Error: No configuration file specified[/red]")
            console.print("\nUsage:")
            console.print("  specify voice --config path/to/voice-config.json")
            console.print("\nOr create voice-config.json in the current directory")
            console.print(
                "Template available at: templates/voice-config.json in the jp-spec-kit repository"
            )
            raise typer.Exit(1)
    ...
    try:
        ...
        console.print("\nPlease check:")
        console.print("  - All required fields are set in config file")
        console.print("  - API keys are set in environment variables")
        console.print("  - See templates/voice-config.json for reference")
        raise typer.Exit(1)
    except Exception as e:
    ...
    console.print(f"  Assistant: {voice_config.assistant.name}")
    console.print(f"  STT Provider: {voice_config.pipeline.stt.provider}")
    console.print(f"  LLM Provider: {voice_config.pipeline.llm.provider}")
    console.print(f"  TTS Provider: {voice_config.pipeline.tts.provider}")
