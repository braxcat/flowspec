...
MCP servers are configured in `.mcp.json` at the project root. The configuration uses `${PWD}` for path portability across machines - see [ADR-005](docs/adr/ADR-005-mcp-path-portability.md) for details.
